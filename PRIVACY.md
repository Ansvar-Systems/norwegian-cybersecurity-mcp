...
When running locally via `npx @ansvar/norwegian-cybersecurity-mcp`, all database queries stay on your machine. The only external data flow is between the MCP client and the AI provider (e.g., Anthropic).
...
- **Tool responses**: Guidance text, advisory content, framework summaries
...
- **General guidance research**: Safe to use through any deployment -- NSM guidance is public information
- **Organisation-specific queries**: If your queries could reveal confidential security posture information (e.g., specific gaps in your security controls), consider using the local npm package
- **Sensitive matters**: For confidential security assessments, use the local deployment option
...
For privacy questions: https://github.com/Ansvar-Systems/norwegian-cybersecurity-mcp/issues
